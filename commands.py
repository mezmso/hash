...
class LoadConfigCommand(command.Command):
    name = 'loadconfig'
    help = 'load aliases (and eventually other things) from a conf file'
    def execute(self, *args):
        conf = ConfigParser.RawConfigParser()
        conf.read(args[0])
        for o in conf.options('alias'):
            v = conf.get('alias', o)
            self.hash.aliascmds[o] = v
            self.hash.cmdtab[o] = alias.Alias(self.hash, v)
